...
const PlatformTable = (props) => {
  ...
  // const onReq = (payload) => getEntranceListPlat({ ...payload, type: 2 })
  ...
  const fetchData=()=>{
    getEntranceListPlat({ type: 2 }).then((res) => {
          ...
          <div>
            <Space>
              <span style={{ color: "#008dff", cursor: "pointer" }}
                ...
                onClick={() => {
                  props.history.push(`entrance/edit?type=edit&tabspane=2&id=${text}`)
                }}
    ...
    <div>
      ...
      {photourl ?
        <Modal visible footer={null} width={500} onCancel={() => { setPhotoUrl() }}
          style={{ display: "flex" }}>
          {
            photourl.map((item, index) => (<img key={index} src={item} alt="图片" width={300} />))
          }
